...
################################################################################
infile = "Taxonomy_working.html"; #preprocessed to remove headers/css etc...
...
for l in buffer:
    if l.strip()=="<UL COMPACT>":
        ...
        extracted.append({
            **{
                'path': '.'.join(path),
                'leafnode': '', #set after taxonomy is processed
            },
            **edict
        });
...
for i in range(0,len(extracted)):
    ...
    if taxon_id in parent_ids:
        extracted[i]['leafnode'] = 'node';
    else:
        extracted[i]['leafnode'] = 'leaf';
...
cols = ['leafnode','path','taxon_id','name','title','type','href','plink','pcount'];
